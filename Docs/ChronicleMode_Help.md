...
3. Click the "Run Autofarm" button (click into the map if the game application is not focused). You can do **windows + tab keys** to switch between the applications.
4. Check if it's working correctly. If the application didn't selecte the Heroic difficulty and the match hasn't started, stop the execution and start again. If it's working correctly, don't move the mouse or switch between applications to avoid any error.
5. Enjoy it!!!
